...
interface AssignToListSheetProps {
  ...
  onDone: (addToMasterRankings: boolean, customListIds: string[]) => void;
  onClose: () => void;
}

export default function AssignToListSheet({
  visible,
  movie,
  onDone,
  onClose,
}: AssignToListSheetProps) {
  const [masterRankingsSelected, setMasterRankingsSelected] = useState(false);
  ...
  React.useEffect(() => {
    console.log('[AssignToListSheet] visible changed to:', visible, 'movie:', movie?.title);
    if (visible) {
      // Reset selection when opening
      setMasterRankingsSelected(false);
  ...
  console.log('[AssignToListSheet] rendered with movie:', movie?.title, 'visible:', visible);

  const handleDone = () => {
    if (!movie) return;
    const customListIdsArray = Array.from(selectedCustomListIds);
    onDone(masterRankingsSelected, customListIdsArray);
  };

  // Always render Modal when visible is true, even if movie is null (it will be set)
  if (!visible) {
    console.log('[AssignToListSheet] Not visible, returning null');
    return null;
  }

  console.log('[AssignToListSheet] Rendering Modal, visible:', visible, 'movie:', movie?.title);
        ...
        >
          ...
          >
            ...
            <View style={styles.header}>
              <Text style={styles.headerTitle}>Assign to lists</Text>
            ...
            <View style={styles.footer}>
              <TouchableOpacity
                style={[
                  styles.doneButton,
                  !masterRankingsSelected && selectedCustomListIds.size === 0 && styles.doneButtonDisabled,
                ]}
                onPress={handleDone}
                activeOpacity={0.8}
                disabled={!masterRankingsSelected && selectedCustomListIds.size === 0}
              >
